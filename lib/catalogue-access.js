...
function readAccessGuids() {
  try {
    ...
    return {
      allowedGuids: Array.isArray(allowlist.allowedGuids)
        ? allowlist.allowedGuids
        : [],
      adminGuids: Array.isArray(allowlist.adminGuid) ? allowlist.adminGuid : [],
    };
  } catch (error) {
    console.error("Unable to read the catalogue GUID allowlist:", error);
    return { allowedGuids: [], adminGuids: [] };
  }
}
