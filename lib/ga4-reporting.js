...
async function runRealtimeReport(body) {
  ...
  const response = await fetch(
    `${reportEndpoint}/properties/${propertyId}:runRealtimeReport`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  const result = await response.json();
  if (!response.ok) {
    throw new Error(result.error?.message || "Google Analytics realtime request failed");
  }
  return result;
}
...
export async function getGa4Report(days = 30) {
  ...
  const [overview, previous, daily, pages, sources, countries, cities, devices, events, realtime] =
    await Promise.all([
  ...
  const result = {
    ...
    realtimeActiveUsers: values(realtime).reduce(
      (total, row) => total + (row.metrics[0] || 0),
      0,
    ),
    realtimeLocations: values(realtime),
